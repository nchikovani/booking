...
_Рекомендуемая модель: Claude 4.6 Sonnet_ — обнаружение рисков и edge cases.
...
### 5. Реализация _(начало чата 2)_
...
_Рекомендуемая модель: Gemini 3 Flash_ — механический запуск команд.
...
_Рекомендуемая модель: Claude 4.6 Sonnet_ — финальная проверка перед коммитом.

### 8. Обновление документации

**`/update-docs`** — вызывает subagent `docs-updater`, который обновляет spec и план, roadmap, при необходимости architecture и README.

_Используемая модель: Gemini 3 Flash_ — редактирование документации.
